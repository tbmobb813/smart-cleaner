...
class TmpCleaner(BasePlugin):
                ...
                if p.is_file():
                    ...
                    items.append(
                        CleanableItem(
                            ...
                            description=f"Temp file: {p.name}",
                    ...
                    items.append(
                        CleanableItem(
                            path=str(p), size=size, description=f"Temp dir: {p.name}", safety=SafetyLevel.SAFE
                        )
                    )
            except Exception:
                continue
    ...
    PLUGIN_INFO = {
        ...
        "class": "TmpCleaner",
        "config": {"base_dir": {"type": "path", "description": "Base temporary directory to scan", "example": "/tmp"}},
        "constructor": {
            "base_dir": {
                "type": "path",
                "default": "/tmp",
